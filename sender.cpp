...
void init(int& shmid, int& msqid, void*& sharedMemPtr)
{
	 ...
	 */
	 ...
	 if(key  == -1)
	 {
		 ...
		 cout<<"The ftok key is successfully created."<<endl;
	 }
...
void send(const char* fileName)
{
	...
	while(!feof(fp))
	{
		...
		/* TODO: Send a message to the receiver telling him that the data is ready
		 ...
		 {
			 cout<<"Successfully send the message."<<endl;
		 }
		/* TODO: Wait until the receiver sends us a message of type RECV_DONE_TYPE telling us
 		 * that he finished saving the memory chunk.
 		 */
		 cout<<" Wait until the receiver sends us a message."<<endl;
		 if(msgrcv(msqid,&rcvMsg,0,RECV_DONE_TYPE,0)==-1)
		 {
			 cerr<<"Fail to wait to receive a message."<<endl;
			 exit(1);
		 }
		 else
		 {
			 cout<<"Successfully receive the message."<<endl;
	...
	/** TODO: once we are out of the above loop, we have finished sending the file.
	  ...
	  */
		sndMsg.size =0;
		sndMsg.mtype = SENDER_DATA_TYPE;
		cout<<"Sending the Empty message."<<endl;
		if(msgsnd(msqid, &sndMsg, sizeof(sndMsg), 0) == -1)
	{
		cout<<"Fail to send Empty message."<<endl;
	}else
	{
		cout<<"Successfully send the empty message."<<endl;
	}
